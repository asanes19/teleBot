...
user_states = {}  # Dictionary to store the state for each user
...
if __name__ == '__main__':
    ...
    async def handle_new_message(event):
        ...
        if event.is_private:
            ...
            from_ = await event.client.get_entity(from_id)

            # Initialize state if not present
            if from_id not in user_states:
                user_states[from_id] = CONVERSATION_STATES['START']
            ...
            if conversation_active:
                if current_state == CONVERSATION_STATES["START"]:
                    if (
                        "1" in message_text
                        or "شراء" in message_text
                        or "شراء usdt" in message_text
                    ):
                        current_state = CONVERSATION_STATES["BUY_SELL_CHOICE"]
                        response = keyword_responses["شراء"]
                    elif (
                        "2" in message_text
                        or "بيع" in message_text
                        or "بيع usdt" in message_text
                    ):
                        current_state = CONVERSATION_STATES["BUY_SELL_CHOICE"]
